# -*- encoding:utf8 -*-
# changwng/dkagh1234
#  https://ticket.interpark.com/Ticket/Goods/GoodsInfo.asp?GoodsCode=19008298
# 일반  08/10
...
# 위젯 설정
 ...
 # 회차
    ...
    e9.addItems(["농협", "BC", "국민", "우리", "씨티", "신한", "우체국", "하나"])

############### 서울올림픽 주경기장, 서울 올림픽공원 KSPO DOME(체조경기장)
    e1.setText("changwng")
    e2.setText("dkagh1234..")
    e3.setText("710725")
    #e4.setText("https://ticket.interpark.com/Ticket/Goods/GoodsInfo.asp?GoodsCode=19008298");
    #http://ticket.interpark.com/Ticket/Goods/GoodsInfo.asp?GoodsCode=19008298
    e4.setText("https://ticket.interpark.com/Ticket/Goods/GoodsInfo.asp?GoodsCode=19011623"); #8월 16일자
    #http://ticket.interpark.com/Ticket/Goods/GoodsInfo.asp?MN=Y&GroupCode=19011623&strGroupCode=19011623 // 올림픽체조경기장 // 2019 K-WORLD FESTA ［개막공연］

    e6.setText("1회차")
    e7.setText("일반") 
    e9.setCurrentIndex(2) #국민카드 디폴트값
    #e9.setText("국민")
    cb4.setChecked(True)  
    cb5.setChecked(True)  
    # 시작버튼
...
# 입력한 정보 받아오기
    # ID, PW, 주민번호 앞자리
    strUserId = e1.text().strip()
    strUserPW = e2.text().strip()
    userNum = e3.text()
    log(strUserId +':'+ strUserPW)
    ...
    # else: userBank = 38051
    # 무통장 입금은 사라져서 카드사이름으로 대체함
    if userBank.find("농협") != -1: userBank = 26013
    elif userBank.find("BC") != -1: userBank = 67
    elif userBank.find("국민") != -1: userBank = 62
    elif userBank.find("우리") != -1: userBank = 64
    #elif userBank.find("기업") != -1: userBank = 38057
    elif userBank.find("씨티") != -1: userBank = 53
    elif userBank.find("신한") != -1: userBank = 26010
    elif userBank.find("우체국") != -1: userBank = 38058
    elif userBank.find("하나") != -1: userBank = 26011
    else: userBank = 38051
...
# 매크로 시작
    ...
    # 인터파크 화면 띄우기
        # 드라이브 객체 생성
        url = 'C:/Python/work/interpark/chromedriver_win32/chromedriver.exe'  # 드라이브가 있는 경로
        driver = webdriver.Chrome(url)

    # 검색어 타입에 따라 루트1, 루트2 분기 발생
    # 루트 1 : 링크로 입력했을 경우
        if ( userSearch.startswith('https://ticket.interpark.com/Ticket') or   userSearch.startswith('http://ticket.interpark.com/Ticket') )  :
            driver.get(userSearch)

    # 루트 2 : 링크가 아닌 타입일 경우
        # 인터파크 화면 띄우기
        else :
            driver.get("https://ticket.interpark.com/?smid1=header&smid2=ticket")
    ...
    # 로그인
        ...
        # ID, PW 입력하기
        #driver.find_element_by_xpath('//*[@id="userId"]').send_keys(strUserId)
        #driver.find_element_by_xpath('//*[@id="userPwd"]').send_keys(strUserPW)
        # oUserId = driver.find_element_by_name('userId'); 
        #oUserId.clear()
        #oUserId.send_keys(strUserId)  # ID 입력
        #oPasswd = driver.find_element_by_name('userPwd')
        #oPasswd.clear()
        #oPasswd.send_keys(strUserPW)  # PW 입력
        ...
        driver.find_element_by_id('userId').send_keys(strUserId)
        driver.find_element_by_id('userPwd').send_keys(strUserPW)

        driver.find_element_by_id('btn_login').click()
        driver.switch_to_default_content()

    #    driver.find_element_by_id("userId").send_keys("changwng")
    # driver.find_element_by_id("userPwd").send_keys("dkagh1234..")  
        # 로그인 버튼 누르기
        ...
        if elem != None :
            ...
            while seatch != True :
                for i in range(0, len(bs4.findAll('area')) + 1) :
                    ...
                    log('현재 페이지에  seatList은 %d개가 있습니다.' % len(seatList)) 
                    ...
                    try:
                        ...
                        for i in range(0, len(seatList)):
                            ...
                            if cbCheck[4] == 1:
                                seatch = True
                                break
                        log('현재 페이지에 좌석  seatch  %b.' %seatch ) 
                        # 좌석 유무를 검사한다.
                        if seatch == True:
                            # 좌석이 있을 경우
                            # 좌석 선택하기
            ...
            try:
                ...
                if bs4.find('map') != None :
                    ...
                    # 구역 리스트 받아오기
                    areaList = bs4.findAll('area')
                    log("미니맵이 존재하지 않을 경우 2 구역이 존재할 경우")
                    # 빈 좌석 찾기
                    seatch = False
                    while seatch != True :
                        for i in range(0, len(areaList)) :
                            # 좌석 프레임 받아오기
                            log("미니맵이 존재하지 않을 경우 3 구역이 존재할 경우  좌석 프레임 받아오기")
                            ...
                            log("미니맵이 존재하지 않을 경우 4 seatList: count1: %d"%len(seatList));
                            # 좌석이 존재할 경우 error X -> except 실행 X
                            try:
                                # 좌석 등급 조건에 따른 가부
                                for i in range(0, len(seatList)):
                                    ...
                                    log("미니맵이 존재하지 않을 경우 5 text:%s"%text)
                                    if (text.find("VIP") != -1) & (cbCheck[0] == 1):
                                        seatch = True
                                        break
                                    if (text.find("R") != -1) & (cbCheck[1] == 1):
                                ...
                                log("seatch: %s "%str(seatch)) 
                                # 좌석 유무를 검사한다.
                                if seatch == True:
                                    # 좌석이 있을 경우
                                    # 좌석 선택하기
                                    log("미니맵이 존재하지 않을 경우 6 좌석 선택하기 script: %s: %s" %(text,seat['onclick']))
                                    #driver.execute_script(seat['onclick'] + ";")
                                    #seat.click()
                                    #driver.execute_script("arguments[0].click();", seat) #클릭에러 발생시 회피동작
                                    
                                    oSeatN = driver.find_element_by_xpath("//span[@title='"+seat['title']+"']")
                                    driver.execute_script("arguments[0].click();", oSeatN)
                                    log("미니맵이 존재하지 않을 경우 6.5 onclick() ")
                                  
                                    # 2단계 프레임 받아오기
                                    log("미니맵이 존재하지 않을 경우 7 2단계 프레임 받아오기")
                                    driver.switch_to.default_content()
                                    frame = driver.find_element_by_id('ifrmSeat')
                                    driver.switch_to.frame(frame)
                                    #time.sleep(30)    
                                    # 3단계 넘어가기
                                    log("미니맵이 존재하지 않을 경우 8 3단계 넘어가기")
                                    driver.execute_script("javascript:fnSelect();")
                            ...
                            # 좌석이 존재하지 않을 경우 error O -> except 실행
                                ...
                                log("에러 발생 %s"%ex1)
                                log("미니맵이 존재하지 않을 경우 9: 좌석이 없을 경우 2단계 프레임 받아오기")
                                # 좌석이 없을 경우
                                # 2단계 프레임 받아오기
                                driver.switch_to.default_content()
                                time.sleep(0.5)  
                                frame = driver.find_element_by_id('ifrmSeat')
                                driver.switch_to.frame(frame)
                                log("미니맵이 존재하지 않을 경우 10: ifrmSeat")
                                # 좌석도 전체보기
    ...
    # 가격/할인선택 (3단계)
        ...
        # 표 선택하기
        # 표 정보를 가져온다.
        #log("source: %s"%driver.page_source)
        bs4 = BeautifulSoup(driver.page_source, "html.parser")
        ticketList = bs4.findAll('select')
        log("가격/할인선택 (3단계) ticketList %d"%len(ticketList))
        # 사용자의 입력값과 일치하는 함수를 찾는다.
        for i in range(0, len(ticketList)):
            ticketStr = ticketList[i]["pricegradename"]
            log("ticketStr taL %s"%ticketStr)
    ...
    # 결제하기_1 (5-1단계)
        ...
        elem = driver.find_element_by_xpath("//tr[@id='Payment_22003']//input[@name='Payment']")
        elem.click()

        # 은행 선택 (무통장입금)
        #elem = driver.find_element_by_xpath("//select[@id='BankCode']//option[@value='" + str(userBank) + "']")
        elem = driver.find_element_by_xpath("//select[@id='DiscountCard']//option[@value='" + str(userBank) + "']") 
        elem.click()
